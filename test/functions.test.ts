...
async function check_release(
  ...
  expect(matched_release.version).toBe(expected_version)
}

test("Major and minor version only (edge newer)", async () => {
  await check_release("v21.04", "v21.04.2", false)
})
test("Major and minor version only (edge older)", async () => {
  await check_release("v21.04", "v21.04.2", true)
})
test("Major, minor, and patch version: latest patch (edge newer)", async () => {
  await check_release("v21.04.2", "v21.04.2", false)
})
test("Major, minor, and patch version: latest patch (edge older)", async () => {
  await check_release("v21.04.2", "v21.04.2", true)
})
test("Major, minor, and patch version: older patch (edge newer)", async () => {
  await check_release("v21.04.1", "v21.04.1", false)
})
test("Major, minor, and patch version: older patch (edge older)", async () => {
  await check_release("v21.04.1", "v21.04.1", true)
})
test("Edge release", async () => {
  await check_release("v21.03.0-edge", "v21.03.0-edge", false)
})

test("Install Nextflow", async () => {
  ...
  expect(version_output).toMatch(version_regex)
})
